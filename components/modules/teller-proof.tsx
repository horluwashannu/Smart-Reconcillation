      ...
      raw.slice(1).forEach((r) => {
        ...
        if (safeNumber(obj["CHEQUES"]) > 0)
          rows.push({
            ...
            WITHDRAWAL: safeNumber(obj["CHEQUES"]),
          })

        // SAVINGS Withdrawal
        if (safeNumber(obj["SAVINGS"]) > 0)
          rows.push({
            ACCOUNT_NO: obj["ACCOUNT_NO_2"] || "",
            WITHDRAWAL: safeNumber(obj["SAVINGS"]),
          })

        // DEPOSIT
        if (safeNumber(obj["DEPOSIT"]) > 0)
          rows.push({
            ACCOUNT_NO: obj["ACCOUNT_NO_3"] || "",
            DEPOSIT: safeNumber(obj["DEPOSIT"]),
          })

        // EXPENSE
        if (safeNumber(obj["EXPENSE"]) > 0)
          rows.push({
            ACCOUNT_NO: obj["ACCOUNT_NO_4"] || "",
  ...
  const parseGL = async (file: File) => {
    try {
      ...
      const rows = raw.slice(1).map((r) => ({
        Date: String(
          r[header.findIndex((h) => h.includes("transaction date"))] || ""
        ),
        ...
        Amount: safeNumber(
          r[
            header.findIndex(
              (h) => h.includes("lcy amount") || h.includes("amount")
            )
          ]
        ),
        User: String(r[header.findIndex((h) => h.includes("user"))] || ""),
        Authorizer: String(
          r[header.findIndex((h) => h.includes("authoriser"))] || ""
        ),
          ...
          <div className="grid md:grid-cols-3 gap-6">
            <div>
              ...
              <Input
                ...
                accept=".xlsx,.xls,.csv"
                onChange={(e) =>
                  e.target.files?.[0] && parseTellerUpload(e.target.files[0])
                }
              />
              {tellerRows.length > 0 && (
