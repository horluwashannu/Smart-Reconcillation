...
interface DenominationRow {
  value: number
  label: string
  bundles: number // Changed to 500 notes per bundle
  pieces: number // individual notes
  packs: number // 1000 pieces per pack
}
...
export function SmartTellerCalculator({ userId }: SmartTellerCalculatorProps) {
  const [nairaDenominations, setNairaDenominations] = useState<DenominationRow[]>(initialNairaDenominations)
  const [dollarDenominations, setDollarDenominations] = useState<DenominationRow[]>(initialDollarDenominations)
  const [gbpDenominations, setGBPDenominations] = useState<DenominationRow[]>(initialGBPDenominations)
  const [euroDenominations, setEuroDenominations] = useState<DenominationRow[]>(initialEuroDenominations)
  ...
  const updateDenomination = (
    ...
    const setter =
      currency === "naira"
        ? setNairaDenominations
        : currency === "dollar"
          ? setDollarDenominations
          : currency === "gbp"
            ? setGBPDenominations
            : setEuroDenominations
  ...
  const calculateCurrencyTotal = (denominations: DenominationRow[]) => {
    return denominations.reduce((sum, row) => sum + calculateRowTotal(row), 0)
  }
  ...
  const clearCurrency = (currency: Currency) => {
    ...
    const initial =
      currency === "naira"
        ? initialNairaDenominations
        : currency === "dollar"
          ? initialDollarDenominations
          : currency === "gbp"
            ? initialGBPDenominations
            : initialEuroDenominations

    setter(initial.map((d) => ({ ...d, bundles: 0, pieces: 0, packs: 0 })))
  }
  ...
  const handleSave = async (currency: Currency, denominations: DenominationRow[], symbol: string) => {
    ...
    try {
      ...
      console.log("[v0] Calculator data saved to Supabase")
            ...
            <tbody>
              ...
              <tr className="border-t-2 border-primary bg-primary/5">
                ...
                <td className="p-3 text-right font-mono text-lg font-bold text-primary">
                  {symbol}
                  {calculateCurrencyTotal(denominations).toLocaleString()}
                </td>
              </tr>
            </tbody>
          </table>
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground">Calculate cash totals by denomination with bundles, pieces, and packs</p>
      </div>
